            ...
            <div className="bid-section">
              ...
              <form onSubmit={handleBidSubmit} className="bid-form">
                <div className="bid-note">
                  💡 <strong>Rules:</strong>
                  <br />
                  • Minimum bid = item value (or current bid + $1)
                  <br />
                  • If item was won: bid must be ≥ 2x winning amount
                  <br />
                  • Cannot bid more than your score
                  <br />
                  • Points refunded if outbid
                </div>
                <div className="bid-inputs">
                  <select
                    value={bidForm.teamName}
                    onChange={(e) =>
                      setBidForm({ ...bidForm, teamName: e.target.value })
                    }
                    required
                    className="bid-select"
                    style={{
                      color: bidForm.teamName ? "#000" : "#999",
                    }}
                  >
                    ...
                    {db.teams.map((team) => (
                      <option key={team.id} value={team.name}>
                        {team.name} (${team.score} available)
                      </option>
                    ))}
                  </select>

                  <select
                    value={bidForm.itemNumber}
                    onChange={(e) => {
                      const itemId = parseInt(e.target.value);
                      const minBid = getMinimumBid(itemId);
                      setBidForm({
                        ...bidForm,
                    ...
                    className="bid-select"
                    style={{
                      color: bidForm.itemNumber ? "#000" : "#999",
                    }}
                  >
                    <option value="" disabled>
                      Select Item (Any Item)
                    </option>
                    {db.goodies.map((goodie) => {
                      const currentBid = getCurrentBid(goodie.id);
                      const minBid = getMinimumBid(goodie.id);

                      return (
                        <option key={goodie.id} value={goodie.id}>
                          {/* Only show name if revealed, otherwise just ID */}
                          {goodie.revealed 
                            ? `#${goodie.id} - ${goodie.name}` 
                            : `Item #${goodie.id}`}
                          {" "}
                          {goodie.winner &&
                            `(Won: $${goodie.winner.bidAmount}, Min: $${minBid})`}
                          {!goodie.winner &&
                            currentBid &&
                            `(Current: $${currentBid.bidAmount})`}
                          {!goodie.winner &&
                  ...
                  <input
                    ...
                    placeholder={
                      bidForm.itemNumber
                        ? `Min: $${getMinimumBid(
                            parseInt(bidForm.itemNumber)
                          )}`
                        : "Bid Amount $"
                    }
                    min={
                      bidForm.itemNumber
                        ? getMinimumBid(parseInt(bidForm.itemNumber))
                        : 1
                    }
                    step="1"
                    required
                ...
                <button
                  type="submit"
                  className="bid-submit-button button-hover"
                >
                  Submit Bid
                </button>
              </form>
              ...
              {/* All Bids History */}
              <h3 className="bid-history-title">All Recent Bids</h3>
              <div className="bid-history-list">
                {db.bids
                  ...
                  .slice(0, 10)
                  .map((bid, index) => {
                    const item = db.goodies.find((g) => g.id === bid.itemId);
                    return (
                      <div
                        key={bid.id}
                      ...
                      >
                        <div className="bid-team-name">{bid.teamName}</div>
                        <div className="bid-item-badge">
                          {item
